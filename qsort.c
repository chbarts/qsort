...
{
    puts("qsort version 0.1.0");
    puts("Copyright 2015 Chris Barts.");
        ...
        if (curlen == curmax) {
            ...
            if (NULL == tmp) {
                ...
                for (i = 0; i < curmax; i++)
                    free(buf[i]);
                free(buf);
                fprintf(stderr, "qsort: memory error\n");
                exit(EXIT_FAILURE);
            } else {
                curmax *= 2;
                buf = tmp;
...
int main(int argc, char *argv[])
{
    ...
    if ((buf = malloc(1024 * sizeof(buf[0]))) == NULL) {
        fprintf(stderr, "qsort: malloc failure\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (1 == numer) {
        quicksort((void **) buf, 0, curlen - 1, ncmp);
    } else {
        quicksort((void **) buf, 0, curlen - 1, cmp);
    }
